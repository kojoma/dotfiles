...
### Pull Request マージ後の整理

- 作業していた Pull Request（PR）がマージされたら、ローカルの `main` ブランチへ変更を取り込みます。
- 取り込み後は、ローカルの作業ブランチを削除します。
- リモートの作業ブランチは削除しません。GitHub リポジトリの設定により自動削除される場合があります。
